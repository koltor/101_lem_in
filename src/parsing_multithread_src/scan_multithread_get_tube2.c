...
/*   Updated: 2019/06/21 18:21:27 by matheme     ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static void	multithread_check_duplicate_tube(t_tube *t_tab, UINT size, UINT start, UINT end)
{
	UINT	i;
	UINT	j;
...
static void	*thread_main(void *arg)
{
    ...
    if (thread.id == NB_THREAD)
        multithread_check_duplicate_tube(thread.data->t_tab, thread.data->tubes, (thread.id - 1) * thread.section, thread.data->tubes);
    else
	    multithread_check_duplicate_tube(thread.data->t_tab, thread.data->tubes, (thread.id - 1) * thread.section, thread.id * thread.section);
	return (NULL);
}
