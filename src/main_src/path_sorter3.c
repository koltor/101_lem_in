...
void	malloc_res_pm(t_data *data, UINT size_tab, UINT (*res)[])
{
	UINT i;
	...
	while (i < size_tab)
	{
		index = get_index_valid_tab(data, res, data->max_paths);
		size_small_tabs = get_size_valid_tab(data, res, data->max_paths);
